...
const AnalogClock = ({ hour, minute, second } : ClockProps) => {
  const hoursDeg = (hour * 30) + (minute / 2);
  const minutesDeg = (minute * 6) + (second / 10);
  const secondsDeg = second * 6;

  const hourStyle = { transform: `rotateZ(${hoursDeg}deg)` };
  const minuteStyle = { transform: `rotateZ(${minutesDeg}deg)` };
  const secondStyle = { transform: `rotateZ(${secondsDeg}deg)` };
  ...
  return (
    <div>
      <div className="clock-container styling">
        <div
          id="clock"
          className={'clock-content waiting'}
        >
          ...
          <div
            id="hours-indicator"
            className={`indicator hours-indicator ${(hour === 0 ? '' : 'transition-effect')}`}
            style={hourStyle}
          />
          <div
            id="minutes-indicator"
            className={`indicator minutes-indicator ${(minute === 0 ? '' : 'transition-effect')}`}
            style={minuteStyle}
          />
          <div
            id="seconds-indicator"
            className={`indicator seconds-indicator ${(second === 0 ? '' : 'transition-effect')}`}
            style={secondStyle}
          />
